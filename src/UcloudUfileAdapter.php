...
use LogicException;
use Omuigix\LaravelUcloudStorage\UfileSdk;
...
class UcloudUfileAdapter extends AbstractAdapter {
	...
	public function __construct($bucket, $public_key, $secret_key, $suffix = '.ufile.ucloud.cn', $pathPrefix = '', $cdn, $https = false) {
	...
	public function writeStream($path, $resource, Config $config) {
		...
		$mime = $config->get('mime', 'application/octet-stream');
		$checkCrc = $config->get('checkCrc', false);
		list($ret, $code) = $this->ufileSdk->putFile($path, $resource, ['Content-Type' => $mime]);
		return $ret;
	}
